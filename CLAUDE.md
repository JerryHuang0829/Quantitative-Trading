...
├── src/utils/constants.py       共用常數（TW_TZ、TW_ROUND_TRIP_COST、MIN_OHLCV_BARS、MOMENTUM_*、REVENUE_LAG_DAYS、TECH_SUPPLY_CHAIN_KEYWORDS）
...
| `scripts/walk_forward.py` | Rolling OOS 滾動驗證（非真正 Walk-Forward） | Docker |
...
| `scripts/refresh_reports.sh` | 一鍵重跑 Rolling OOS + Dashboard | Docker |
...
| `scripts/regime_simulation.py` | Regime 改進模擬（研究用，不影響生產） | Windows |
...
| `/walk-forward` | Rolling OOS 滾動驗證 + 分析 | Docker | default |
...
| Agent | 用途 |
|-------|------|
| `quant-analyst` | 多步策略分析（診斷 Rolling OOS 視窗、比較回測、因子評估） |
...
**已完成**：P0-P3 策略研究 + P4.0-P4.6/P4.8/P4.9 工程化 + P5 五輪雙視角審查 + P6 六輪雙視角審查 + 全專案盲點修正 Phase 1-4（20 項）+ 雙視角全專案審查（APPROVE ✅）
...
- TWSE 1,077 支 + TPEX 881 支，資料到 **2026-04-15**
...
**2026-04-14 新增（0050 Split Fix）**：
- 發現 0050 於 2025-06-18 進行 1:4 股票分割（188.65→47.57），`_analyze_market_proxy()` 使用原始 OHLCV 計算 SMA/ADX，分割後 close < SMA60 → 6/7/8 月假 risk_off
- 修復：`tw_stock.py` 在 `_analyze_market_proxy()` 對 0050 OHLCV 呼叫 `adjust_splits()` 前復權後再計算指標
- 新增 `scripts/regime_simulation.py`（研究用，模擬 3 種 regime 改進方案 + 分割校正效果）
- 回測驗證：4Y Sharpe 0.84→0.97、2025 OOS Alpha -10%→+7.27%、161 tests passed

**2026-04-15 全專案盲點修正 Phase 1-4 + 雙視角審查**：
- 20 項修正：S1-S5（策略層）+ D1-D5（資料層）+ O1-O3（運維層）+ C1-C4（程式碼品質）
- C1：7 個常數統一到 `constants.py`（TW_ROUND_TRIP_COST、MIN_OHLCV_BARS、MOMENTUM_PERIOD_3M/6M/12M、MOMENTUM_SKIP_DAYS、REVENUE_LAG_DAYS）
- C2：刪除未使用的 `src/data/binance.py`（77 行）
- C3：`_rank_analyses()` + `_select_positions()` 決策 logging
- C4：tw_stock.py magic numbers 替換為 named constants
- O3：walk_forward.py 正名為 Rolling OOS Validation
- D5：TPEX 偵測改用 stock_info CSV `type=="tpex"`（取代 index_name 偵測）
- 審查後修正：profile 殘留舊值對齊、停用因子 as_of WARNING、.env.example 補齊、TWSE close>0 過濾
- 雙視角審查結論：**APPROVE ✅ — 無 P0 級問題**

**待做（按優先度）**：
1. 持續累積 Paper Trading 數據（2026-10 正式評估）
