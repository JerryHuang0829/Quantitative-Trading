...
| `scripts/cache_fill.py` | Cache 補齊/更新（`--refresh-all` / `--top-80-only`） | Docker |
...
**最新修改（P4.5 + P4.6）**：
- P4.5：Total Return Benchmark — TWSE 除息爬蟲 + `adjust_dividends()` + scale-invariant 公式
- P4.6：Drift-aware 日報酬 — buy-and-hold within period，含現金部位
- 4Y Sharpe 1.33 → 0.90（更真實），Alpha 23% → 18%（benchmark 加回配息後），Benchmark 年化 5.14%
- Walk-Forward mean Sharpe 1.22 → 1.09，median 0.98 → 1.10，勝率 55%
- 161 個測試全通過（含 14 個新增測試）

**待做（按優先度）**：
1. 持續累積 Paper Trading 數據（2026-10 正式評估）
2. P4.5 已知限制：「權息」記錄的現金股利部分被跳過（P1 #2），影響極小（年 ~0.1-0.3%）

**已完成調查**：W2 428% Alpha — 非程式 bug。原因：drift-aware 在 2021-H1 強趨勢市場放大集中持股漲幅（半年年化 444% return）。W2 期間無「權息」記錄，修復前後數字不變屬正常。
