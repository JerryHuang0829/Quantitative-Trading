...
| `scripts/cache_rebuild.py` | Cache 全新重建（`--phase N` / `--status`，TWSE+TPEX+FinMind） | Docker |
| `scripts/cache_health.py` | 資料完整性報告（top-80 覆蓋率） | Docker |
| `scripts/cache_fill.py` | Cache 增量更新（`--refresh-all` / `--top-80-only`，重建後備用） | Docker |
...
- **新 cache（`data/cache_new/`）**：OHLCV 全部來自交易所原始 API（TWSE STOCK_DAY_ALL + TPEX dailyQuotes），Revenue 來自 FinMind 多 token
...
161 個測試，分佈在 13 個檔案。conda env "quant"（Python 3.12）全通過，Docker 也全通過。
...
- **必要環境變數**：`FINMIND_TOKEN`（API）、`FINMIND_TOKEN2` / `TOKEN3` / `TOKEN4`（多帳號輪替，cache_rebuild 用）、`TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID`（通知，可選）
...
**Cache 全新重建（進行中）**：
- 背景：舊 cache 混合 FinMind + TWSE，FinMind 免費版 971 支股票抓不到（其中 20+ 支超過 top-80 門檻）
- Phase 1 ✅ stock_info 1,961 支 + dividends 6,699 筆
- Phase 2 ✅ TWSE OHLCV 1,074 支（STOCK_DAY_ALL via SOCKS5 proxy，~3hr）
- Phase 3 ✅ TPEX OHLCV 881 支（TPEX 官方 dailyQuotes，~35min）
- Phase 4 ✅ Revenue 1,952 支（FinMind 多 token + proxy 輪替）
- Phase 5 ⏳ market_value（本地計算，< 1 分鐘）
- Top-500 revenue 覆蓋率 100%，3 支 DR 股無資料（正常）
- 完成後：`mv data/cache data/cache_old && mv data/cache_new data/cache` → cache_health → 回測比對

**P4.5 + P4.6**：
- 4Y Sharpe 1.33 → 0.90（drift-aware + total return），161 個測試全通過

**待做（按優先度）**：
1. 完成 Phase 5 + cache 切換 + 回測驗證
2. 持續累積 Paper Trading 數據（2026-10 正式評估）

**不急**：P4.7 FinMind as_of、P4.10 券商對接、P4.11 AI 整合
