...
- `round_trip_cost` = 0.0047（`TW_ROUND_TRIP_COST` in `constants.py`）、`slippage_bps` 預設 10
- `adjust_splits()`：-40% / +100% 門檻
- `detect_regime()`：ADX + SMA → 96%/70%/35%
- **0050 Split Fix**（2026-04-14）：`_analyze_market_proxy()` 對 0050 OHLCV 呼叫 `adjust_splits()` 前復權後再計算 SMA/ADX，修復 2025-06-18 的 1:4 分割導致 3 個月假 risk_off
...
## 4.8 全專案盲點修正 Phase 1-4（2026-04-15，請驗證）

Claude 完成 20 項系統性修正 + 雙視角審查（APPROVE ✅），**請驗證以下關鍵變更**：

**常數統一**：
- `constants.py` 新增 7 個常數：`TW_ROUND_TRIP_COST`、`MIN_OHLCV_BARS`、`MOMENTUM_PERIOD_3M/6M/12M`、`MOMENTUM_SKIP_DAYS`、`REVENUE_LAG_DAYS`
- `tw_stock.py` 和 `engine.py` 是否正確 import 並使用這些常數？是否還有 hardcoded 值殘留？
- `src/data/binance.py` 是否已刪除？是否有其他檔案 import 它？

**決策 logging**：
- `_rank_analyses()` 是否記錄 active factors 和 top-10 ranked？
- `_select_positions()` 是否記錄 selection result（entries/holds/exits/exposure）？

**Profile 對齊**：
- `tw_3m_stable` profile 的 `max_same_industry`、`price_momentum`、`institutional_flow` 是否與 `settings.yaml` 一致？
- Config merge chain：DEFAULT → profile → settings.yaml，最終值是否正確？

**TPEX 偵測**：
- `cache_fill.py` 的 TPEX 偵測是否從 `index_name=="timestamp"` 改為 `stock_info CSV type=="tpex"`？
- `--daily-tpex` 是否仍用舊的 `index_name` 偵測？（應改為一致）

**Walk-Forward 正名**：
- `walk_forward.py` docstring 是否正確說明這是 Rolling OOS（非真正 Walk-Forward）？
...
| 指標 | 新 Cache + Split Fix（2026-04-15，Phase 1-4 修正後） | 你跑出來的 |
|------|----------------------------------|-----------|
| 4Y Sharpe（2022-2025） | 0.97 | ？ |
| 4Y Alpha | +4.91% | ？ |
| 4Y MDD | -32.19% | ？ |
| 2025 OOS Sharpe | 1.88 | ？ |
| 2025 OOS Alpha | +7.27% | ？ |
| Benchmark 年化（4Y） | 15.93% | ？ |
| Benchmark 年化（2025） | 37.34% | ？ |
| WF mean Sharpe | 1.09 | ？ |
| WF Bootstrap 95% CI | [-0.13, 2.41] | ？ |

> ⚠️ 2026-04-14 修復 0050 1:4 股票分割 Bug（`tw_stock.py` `_analyze_market_proxy()` 加 `adjust_splits()` 前復權）。修復前：4Y Sharpe 0.84、2025 Alpha -10%。修復後數字如上。
> ⚠️ 2026-04-15 完成全專案盲點修正 Phase 1-4（20 項：S1-S5 + D1-D5 + O1-O3 + C1-C4）+ 雙視角審查（APPROVE ✅）。重點：常數統一到 `constants.py`（7 個新常數）、刪除 `binance.py`、決策 logging、profile 對齊。KPI 不變。
