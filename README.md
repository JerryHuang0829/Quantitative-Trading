...
scripts/walk_forward.py              (Walk-Forward 滾動驗證)
...
- [scripts/walk_forward.py](./scripts/walk_forward.py) — Walk-Forward 驗證（滾動 18M 訓練 + 6M 測試）
...
- [src/utils/constants.py](./src/utils/constants.py) — 共用常數（TW_TZ、TECH_SUPPLY_CHAIN_KEYWORDS）
...
- 2025 OOS 驗證已完成（Sharpe 1.81 vs IS 1.85，衰減僅 2%）
- Benchmark 為 price_only（含 stock split 自動前復權），Alpha 約高估 2-3%/年（殖利率未計入）
...
## 目前回測結果（2026-04-02 更新，含 OOS）

| 回測期間 | 年化報酬 | Sharpe | Alpha | MDD | Beta | 性質 |
|---------|---------|--------|-------|-----|------|------|
| 6M（2024-H2）| 31.78% | 1.08 | +13.52% | -21.50% | 0.53 | In-Sample |
...
OOS Sharpe 1.81 vs IS 1.85，衰減僅 2%。Benchmark（0050）含 stock split 自動前復權。詳見 `優化紀錄.md`。
...
- ~~**P4.3** Walk-forward 驗證框架~~ ✅ 已完成（11 視窗，平均 Sharpe 1.38）
...
- ~~**P7** 測試擴充 + Bug 修復~~ ✅ 完成（135 測試、finmind bug fix、Dashboard 6M 重跑）
- **P4.5** Total return benchmark（含配息再投資）
- **P4.6** Drift-aware 日報酬計算
- **P4.10** 券商對接

詳見 `優化建議.md`。

**架構評估結論（2026-04-02，Claude + Codex 交叉驗證）：** 策略方向正確、研究有紀律、工程品質中上。135 個測試覆蓋核心模組（含 BacktestEngine 整合測試、finmind 單元測試、rebalance 假日對齊測試），CSV fallback 提升備援能力。剩餘短板：Total return benchmark（P4.5）、drift-aware 日報酬（P4.6）。詳見 `優化建議.md`。
