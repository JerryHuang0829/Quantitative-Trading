...
- [src/backtest/metrics.py](./src/backtest/metrics.py) — KPI 計算（Sharpe / Alpha / MDD 等）+ 報告生成
...
- 尚未接券商 / 下單執行
- 尚未有 paper trading 驗證流程（P2 落地後下一步）
- 回測結果為 in-sample，需 out-of-sample 驗證
...
## 目前回測結果（P2 落地後，2026-03-31）

| 回測期間 | 年化報酬 | Sharpe | Alpha | MDD | Beta | benchmark |
|---------|---------|--------|-------|-----|------|-----------|
| 6M（2024-H2）| 31.78% | 1.08 | +13.52% | -21.50% | 0.53 | 0050 price-only |
| 3Y（2022-2024）| 53.57% | 1.85 | +48.43% | -21.50% | 0.52 | 0050 price-only |

P1（`max_same_industry` 2→3）+ P2（`institutional_flow` 10%→0%）均通過 Codex + Claude 雙重驗證。跨機器驗證通過（差異 < 5%）。詳見 `優化紀錄.md`。

## 下一步建議

- ~~**P1** 6M 負 alpha 根因分析~~ ✅ 已完成（`max_same_industry` 2→3）
- ~~**P2** 因子調整~~ ✅ 已完成（`institutional_flow` 移除）
- **P3** 策略擴展（revenue_momentum 覆蓋率、產業權重限制、position sizing、exit framework）
- **P4** 工程化（paper trading → 券商對接 → AI 整合）
