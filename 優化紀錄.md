...
最後更新：2026-04-08（Cache 全新重建 Phase 1-4 完成）
...
| **Cache 重建** | **全新 cache（TWSE+TPEX 原始 + FinMind Revenue），Phase 1-4 完成** | **⏳** |
...
### Cache 全新重建（2026-04-08）

#### 背景

現有 `data/cache/` 經歷多次 FinMind + TWSE 混合抓取，資料來源不一致。FinMind 免費版有 971 支股票永遠抓不到（其中 20+ 支超過 top-80 門檻），是策略盲區。決定建立全新 cache（`data/cache_new/`），OHLCV 完全不依賴 FinMind。

#### 資料來源架構

| 資料 | 來源 | 技術 |
|------|------|------|
| OHLCV 上市 1,074 支 | TWSE STOCK_DAY_ALL | SOCKS5 proxy pool（HiNetCDN 封鎖 Docker IP） |
| OHLCV 上櫃 881 支 | TPEX 官方 dailyQuotes | 直連（不需 proxy） |
| Revenue 1,952 支 | FinMind | 4 token 順序輪替 + proxy 換 IP |
| stock_info 1,961 支 | TWSE + TPEX OpenAPI | 直連 |
| dividends 6,699 筆 | TWSE TWT49U | 直連 |

#### 技術要點

1. **SOCKS5 Proxy Pool**：TWSE HiNetCDN 封鎖 Docker 出口 IP → 腳本自動掃描 150 個免費 SOCKS5 proxy，取前 3 個可用
2. **STOCK_DAY_ALL**：全市場日快照端點，每日一次請求取得所有上市股 OHLCV。1,897 個交易日 ~3 小時完成（vs 原估逐股逐月 ~38 小時）
3. **TPEX dailyQuotes**：原計畫用 FinMind，改為 TPEX 官方 API。1,897 日 ~35 分鐘完成
4. **FinMind 多 Token**：FinMind 限制 600 req/hr/IP（非 per token）。4 個帳號 token 搭配不同 IP（T4 直連 + T1/T2/T3 各用 SOCKS5 proxy）。透過 `source.loader._FinMindApi__session.proxies.update(px)` 注入 proxy
5. **DR 股**：3 支 DR（9103/9110/9136）FinMind 無 revenue 資料（API 回傳空），屬正常

#### 完成狀態

| Phase | 內容 | 狀態 | 耗時 |
|-------|------|------|------|
| 0 | 建立 cache_new 目錄 | ✅ | 即時 |
| 1 | stock_info + dividends | ✅ | < 1 分鐘 |
| 2 | TWSE OHLCV（1,074 支 × 1,897 日） | ✅ | ~3 小時 |
| 3 | TPEX OHLCV（881 支 × 1,897 日） | ✅ | ~35 分鐘 |
| 4 | Revenue（1,952 支） | ✅ | 多 token 輪替 |
| 5 | market_value（本地計算） | ⏳ | < 1 分鐘 |

#### 資料驗證

- TPEX OHLCV 全面掃描：0 NaN、0 OHLC 異常、0 重複日期
- Top-80/200/500 revenue 覆蓋率：100%
- Revenue 不足 12 個月：61 支（全為 2025+ IPO，資料完整）
- Revenue 無資料：3 支 DR 股（正常）

#### 腳本：`scripts/cache_rebuild.py`

- `--phase N`：分段執行（0-5）
- `--status`：查看各 Phase 進度
- 各 Phase 獨立進度檔（`cache_rebuild_p2.json` / `p3.json` / `p4.json`），可中斷恢復
- Phase 2 和 Phase 3 可同時跑（不同 API、不同進度檔）
