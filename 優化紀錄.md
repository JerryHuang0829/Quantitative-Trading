...
| 驗證項 | 結果 |
|--------|------|
| Docker 測試 | **147 passed, 0 failed**（含 12 個新增 P7 測試 + 修正 fixture） |
| 6M 回測 | Sharpe 0.45（成交金額排序生效） |
| 4Y 回測 | Sharpe 1.33, Alpha +23.28% |
| Walk-Forward | 11/11 通過，平均 Sharpe 1.15，勝率 73% |
| TWSE 市值計算 | 229,874 rows 正常產出 |
...
未覆蓋：`fetch_twse_issued_capital()` 單元測試、`_compute_market_value_from_twse()` 單元測試、`build_tw_stock_universe()` size proxy 路徑直接測試。
